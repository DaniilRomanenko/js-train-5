...
// Завдання 6: Функція приймає деструктуризований масив чисел, де перший елемент - це окремий елемент, а решта - масив.
...
// Перетворюємо масив в рядок за допомогою методу toString() та повторюємо його 2 рази. Результат виводимо на консоль.
function printStrings(array) {
  // Перетворюємо масив в рядок, видаляємо пробіли та виводимо його на консоль
  const resultString = array.toString().replace(/ /g, '');
  
  // Виводимо рядок, повторений 2 рази, на консоль
...
// За допомогою методу toString() перетворюємо масив в рядок і визначаємо, чи є в цьому рядку підрядок subString.
function concatenateStrings(array, subString) {
  ...
  const containsSubstring = arrayString.includes(subString);
  
  // Виводимо результат
  console.log(`The string "${arrayString}" ${containsSubstring ? 'contains' : 'does not contain'} the substring "${subString}"`);
}

console.log("Завдання 8 ====================================");
concatenateStrings(["one", "two", "three"], "two"); // Виведе "The string "one,two,three" contains the substring "two""
